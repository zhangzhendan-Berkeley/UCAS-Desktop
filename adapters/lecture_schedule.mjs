...
async function retryRead(page, config, logger, attempts = 3) {
  let last;
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      logger?.warn?.('lecture.query.retry', { path: 'lecture', attempt, error: String(error?.message || error) });
...
export async function queryScienceSchedule(config, logger) {
  ...
  try {
    ...
    const result = await retryRead(page, config, logger);
