...
def send_mail(account, booking=None, test=False, smtp_factory=None, recipient=None):
    ...
    phase = '连接'
    try:
        smtp = (smtp_factory or smtplib.SMTP_SSL)(host, PORT, timeout=20, context=ssl.create_default_context())
        phase = '认证'
        if host == 'smtp.qq.com':
            smtp.ehlo_or_helo_if_needed()
            if 'LOGIN' not in smtp.esmtp_features.get('auth', '').upper().split():
                raise smtplib.SMTPNotSupportedError('LOGIN unavailable')
            smtp.user, smtp.password = address, account['password'].strip()
        ...
        raise ValueError(f'邮箱登录失败（SMTP {exc.smtp_code}，未发送）：QQ 请确认 SMTP 服务已开启，使用对应发件邮箱的授权码；国科大请检查客户端专用密码。若有临时登录限制，请稍后重试。') from None
    except (smtplib.SMTPSenderRefused, smtplib.SMTPRecipientsRefused, smtplib.SMTPDataError):
        raise RuntimeError('未发送：邮件服务器明确拒绝发件人、收件人或邮件内容，请检查地址及邮箱限制。') from None
    except (smtplib.SMTPException, OSError) as exc:
        if phase != '发送':
            raise RuntimeError(f'未发送：邮件{phase}阶段失败（{type(exc).__name__}），请检查网络、SMTP 服务和发件端授权码。') from None
        raise RuntimeError('发送结果未确认（' + type(exc).__name__ + '）：连接在提交邮件期间中断，请先核对收件箱再决定是否重发。') from None
    finally:
        # QUIT is cleanup. Its failure must neither mask an AUTH error nor undo
        # the server's successful DATA acknowledgement and trigger duplicate mail.
        if smtp is not None:
            try: smtp.quit()
