...
- macOS 可导入未来七天课程与讲座；已预约及明确无需报名的讲座可导入，需要报名但尚未报名的场次排除。
- 修复学校“已经预约过”状态识别，以及无课日期返回 `STATUS=2` 被误报为课程查询失败的问题。
- 包含今日讲座、有效学时、到场提醒、分页查询和验证码等待等此前主分支更新。

升级前从菜单栏完全退出程序，备份 `data/`，然后执行：
...
升级后刷新讲座与听讲记录，再导入日历。自动任务需保持电脑唤醒、应用运行；旧日历事件不会自动删除。详细安装见 [macOS 安装指南](docs/macOS安装.md)。

## Windows 免安装便携版（v0.4.0）

前往 **[GitHub Releases 下载 Windows x64 ZIP](https://github.com/zhangzhendan-Berkeley/UCAS-Desktop/releases/tag/v0.4.0)**，全部解压后双击 **UCAS桌面助手.exe**。已内置 Python、Node 与所需依赖，不用安装开发环境，不用配置环境变量，也不用执行下面的源码安装命令。
...
## v0.5.0 包含：今日讲座、到场提醒与查询修复

新增左侧“今日讲座”，显示两类讲座安排、学校有效听讲次数与学时进度（个人目标 20 学时），支持可开关、可按场次自定义提前时间的本机弹窗提醒。原讲座签到入口已移除。此功能尚未包含在 v0.4.0 发行包中，当前请使用主分支源码；详细操作见下方“今日讲座与到场提醒”。
...
启用科研讲座自动报名后，每小时 01 分查询并尝试报名，仅选择雁栖湖场次；同一开始时间只报名一场，并优先人工智能、机器学习、深度学习、神经网络、自然语言处理、计算机视觉、机器人和具身智能等关键词。报名尚未开放或人数已满时会记录失败并等待下一小时再次查询，不能保证学校名额或网络状态。
...
| 今日讲座 | 人文 / 科研日程、有效次数与学时、可自定义的到场弹窗提醒 | 需同步学校列表；不提供讲座签到 |
| 人文讲座预约 | 仅雁栖湖，按星期/开始时段筛选、报名、01 分检查及发布时间观察 | 可选外部模块；需要 SEP 登录，有时需要邮箱验证 |
...
1. 在“个人信息”保存本人轻新课堂学号与密码，再打开“课程签到”。
...
左侧 **今日讲座** 集中显示今天的人文、科研讲座，按北京时间排序，用不同颜色区分类别，并标注地点、即将开始 / 进行中 / 已结束。原讲座二维码签到和定时签到入口已移除。

1. 在“个人信息”保存 SEP 账号，点击 **刷新讲座与听讲记录**，或在概览点击 **一键刷新全部信息**。两类预告均自动翻页，直到读取到今天之前的场次；有效听讲记录会翻完所有页再累计学时。
2. 顶部显示每类讲座的有效次数、**有效学时 / 20 学时**和还差多少学时。学时仅累加学校明细中“是否有效”已勾选的记录，不按次数换算。学时未完整读取时显示“未获取”，不会把未知当成零。20 学时为个人展示目标；人文页面说明有效听讲 10 次满足要求，最终认定以学校为准。
3. 打开 **开启讲座弹窗提醒**，设置默认提前时间（0–1440 分钟，默认 20 分钟；0 表示开始时）。各场次可取消“提醒本场”，或切换“单独设置提前时间”。设置按 SEP 账号自动保存。
4. 点击 **测试弹窗**预览。提醒窗口显示类型、名称、时间与地点，可以直接打开今日讲座；同一场次重启后不会重复提醒。电脑恢复运行时，尚未开始且已进入提醒时段的讲座会补提醒，已经开始的不会补发。
5. 列表显示所有校区，不代表已预约；请关闭不准备参加的场次提醒，并按现场要求完成考勤。人文自动报名仍只允许雁栖湖。

提醒使用最近一次成功同步的列表，学校有新增或变更时请刷新。首次启用前务必同步；退出程序、休眠和关机期间不会弹窗，关闭窗口进入托盘后会继续。提醒是本机功能，不会提交报名或签到。
...
3. 勾选“每小时 01 分检查并记录新讲座”。需要自动报名时，再勾选“同时自动报名符合筛选条件的雁栖湖讲座”；不勾选则只观察。
4. “检查小时”默认 `0,1,2,...,23`，即每天最多 24 轮，例如 08:01、09:01、10:01；点击 **保存定点计划**。检查之间不运行浏览器、不持续请求学校网站，后台轮次使用无窗口浏览器。
...
7. 点击 **查看发布时间观察** 查看最新报告。满一周后报告提示可以参考时点分布调整“检查小时”，例如 `8,9,12,18`；仍在这些小时的 01 分查询。不会仅凭一周经过就擅自缩减时段。
8. 观察会记录当前列表页的所有校区，但时点分布只统计雁栖湖；提交还受星期、开始时段、学校配额和上游终态记录约束。达到报名配额仍继续只读观察。

**首次发现时间不是准确发布时间。** 报告保留“上次成功检查—首次发现”的观察区间；每小时取样、分页、补放名额、登录失败和关机都可能影响判断。当前只读取列表当前页，不能保证覆盖全部分页或捕获检查间隔内迅速报满的讲座。
...
SEP 账号检测、已选课程同步、选课查询/执行、人文与科研讲座查询、轮询报名统一使用最小化浏览器。普通查询不主动置前；验证码自动识别失败或需要邮箱/新设备验证时，恢复当前浏览器供手工处理，完成后重新最小化。保留同一会话，不因切换窗口重新提交报名或选课。慕课浏览器行为不受此设置影响。

采用最小化有界面浏览器，而非运行中切换无头模式；窗口可从任务栏手动打开。窗口状态由 Chromium 的 [Browser.setWindowBounds](https://chromedevtools.github.io/devtools-protocol/tot/Browser/#method-setWindowBounds) 控制。学校验证码是仍可能弹窗的情况。此改动需更新源码和讲座补丁，再退出旧 App 重启；旧 v0.5.1 源码附件不包含此修复。
