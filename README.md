...
如果使用首次发布版本并遇到“未找到人文讲座报名入口”，先退出应用，运行 `git pull --ff-only`，再执行 `python scripts/setup.py --with-external-modules`。安装器从固定原始版本重新生成组件；旧源文件先备份，不再反复叠补丁。
...
采用最小化有界面浏览器，而非运行中切换无头模式；窗口可从任务栏手动打开。窗口状态由 Chromium 的 [Browser.setWindowBounds](https://chromedevtools.github.io/devtools-protocol/tot/Browser/#method-setWindowBounds) 控制。学校验证码是仍可能弹窗的情况。v0.5.2 已包含此修复；升级后需退出旧 App 重启。
