...
1. 在“人文讲座预约”填写 SEP 账号，再到“课程与讲座签到”点击 **查询科研讲座时间表**（需安装讲座外部依赖）。查询只读取当前列表页，不报名或签到。
...
4. 匹配失败时，仍需该场现场二维码；选课系统的讲座编号和轻新课堂 `courseSchedId` 不是同一套编号，不能直接互换。当前查询和详情页未发现可靠的公开映射。
