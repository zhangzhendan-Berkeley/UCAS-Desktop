...
- Windows 10/11 x64；浏览器任务使用系统 Edge，首次 SEP 浏览器任务可能自动联网下载对应驱动。
...
2. 勾选“每天 08:00 自动查课表并安排签到”，设置“开课前”分钟数，点击 **保存每日计划**。
3. 以后每天按电脑本地时间 08:00 查询当天课表，将所有未结束、未签到的排课加入定时任务，不再需要每天手工勾选。请将 Windows 时区设为北京时间。
...
后台课表查询结果会显示在任务日志，并更新界面中当天的课程列表。应用退出、关机和休眠期间不执行，不会唤醒电脑。普通课程任务与每日任务共享提交去重记录。
...
仅当 `STATUS=0` 且 `stuSignStatus=1` 才记为成功。每节最多 3 次尝试，仅对明确的时间/过期拒绝至少间隔 2 分钟重试（若尚未开课，下一次不早于开课时间）；网络错误和未知结果停止处理。不同任务不会重复加入同一排课。
...
数据仅保存在本机：`data/automation.json` 为不含密码的计划，`data/automation-state.json` 为触发记录，`data/sign-ledger.sqlite3` 为签到提交记录，`data/lecture/<账号哈希>/observations.json` 与 `发布时间观察.md` 为观察数据。账号继续由 Windows DPAPI 加密，以上数据和日志均不提交到 GitHub。
...
- SEP 与轻新课堂账号统一在 **个人信息** 页维护，默认使用 Windows DPAPI 加密保存；填写完整后离开输入框会自动保存，也可点击“保存账号”。重新打开自动读取，旧的已加密账号直接沿用。
