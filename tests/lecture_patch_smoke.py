...
with tempfile.TemporaryDirectory() as tmp:
    ...
    assert 'scienceMode: z.boolean().optional()' in (dest / 'src/config.ts').read_text(encoding='utf-8')
    assert 'if (config.scienceMode && config.onePerStartTime)' in (dest / 'src/workflow.ts').read_text(encoding='utf-8')
    assert 'departmentCell.includes("本科部")' in (dest / 'src/lecture-page.ts').read_text(encoding='utf-8')
